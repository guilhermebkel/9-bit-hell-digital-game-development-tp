...
UIHealthBarWidget::UIHealthBarWidget(Game* game, const Vector2& position, const Vector2& size)
    : mPosition(position)
    , mSize(size)
{
    mFrameActor = new Actor(game);
    mFrameActor->SetPosition(mPosition);
    new StaticSpriteComponent(mFrameActor, "../Assets/HealthBarEmpty.png", mSize.x, mSize.y, HUD::DRAW_ORDER);

    mFillActor = new Actor(game);
    mFillActor->SetPosition(mPosition);
    mFillRect = new RectComponent(mFillActor, mSize.x - 8.0f, mSize.y - 8.0f, RendererMode::TRIANGLES, HUD::DRAW_ORDER + 1);
